...
LPICONRESOURCE CIcons::ReadIconFromICOFile( LPCTSTR szFileName )
{
    ...
    {
        MessageBox( NULL, _T("图标文件打开时出错!"), szFileName, MB_OK );
        return NULL;
    }
    // Allocate memory for the resource structure
    if( (lpIR = (LPICONRESOURCE)malloc( sizeof(ICONRESOURCE) )) == NULL )
    {
        MessageBox( NULL, _T("图标分配内存时出错!"), szFileName, MB_OK );
        CloseHandle( hFile );
        return NULL;
    }
    // Read in the header
    ...
    {
        MessageBox( NULL, _T("读图标文件头时出错!"), szFileName, MB_OK );
        CloseHandle( hFile );
        free( lpIR );
        return NULL;
    }
    ...
    {
        MessageBox( NULL, _T("图标内存重新分配出错!"), szFileName, MB_OK );
        CloseHandle( hFile );
        free( lpIR );
        return NULL;
    }
    ...
    lstrcpy( lpIR->szOriginalDLLFileName, _T("") );
    // Allocate enough memory for the icon directory entries
    if( (lpIDE = (LPICONDIRENTRY)malloc( lpIR->nNumImages * sizeof( ICONDIRENTRY ) ) ) == NULL )
    {
        MessageBox( NULL, _T("图标文件内存重新分配出错!"), szFileName, MB_OK );
        CloseHandle( hFile );
        free( lpIR );
        return NULL;
    }
    ...
    {
        MessageBox(  NULL, _T("读图标文件时出错!"), szFileName, MB_OK );
        CloseHandle( hFile );
        free( lpIR );
        return NULL;
    }
    ...
    for( i = 0; i < lpIR->nNumImages; i++ )
    {
        ...
        {
            MessageBox( NULL, _T("图标内容定位时出错!"), szFileName, MB_OK );
            CloseHandle( hFile );
            free( lpIR );
            free( lpIDE );
            return NULL;
        ...
        {
            MessageBox( NULL, _T("读图标内容出错!"), szFileName, MB_OK );
            CloseHandle( hFile );
            free( lpIR );
            free( lpIDE );
            return NULL;
        ...
        {
            MessageBox( NULL, _T("图标内部格式转换出错!"), szFileName, MB_OK );
            CloseHandle( hFile );
            free( lpIDE );
            free( lpIR );
            return NULL;
...
LPICONRESOURCE CIcons::ReadIconFromEXEFile( LPCTSTR szFileName,LPTSTR lpID )
{
    ...
    if( (hLibrary = LoadLibraryEx( szFileName, NULL, LOAD_LIBRARY_AS_DATAFILE )) == NULL )
    {
        ...
        MessageBox(NULL , _T("装入文件时出错 - 请选择一个WIN32的DLL或EXE文件!"), szFileName, MB_OK );
        return NULL;
    }
    // Store the info
    ...
    if( lpID != NULL )
    {
        ...
        {
            MessageBox( NULL,_T("内存分配出错!"), szFileName, MB_OK );
            FreeLibrary( hLibrary );
            return NULL;
        }
        ...
        lstrcpy( lpIR->szOriginalICOFileName, _T("") );
        ...
        for( i = 0; i < lpIR->nNumImages; i++ )
        {
            ...
            if( ! AdjustIconImagePointers( &(lpIR->IconImages[i]) ) )
            {
                MessageBox(NULL, _T("转换成图标内部格式时出错!"), szFileName, MB_OK );
                free( lpIR );
                FreeLibrary( hLibrary );
                return NULL;
            }
...
BOOL CIcons::WriteIconToICOFile( LPICONRESOURCE lpIR, LPCTSTR szFileName )
{
    ...
    {
        MessageBox( NULL , _T("文件建立时出错!"), szFileName, MB_OK );
        return FALSE;
    }
    // Write the header
    if( ! WriteICOHeader( hFile, lpIR->nNumImages ) )
    {
        MessageBox( NULL , _T("写图标文件头时出错!"), szFileName, MB_OK );
        CloseHandle( hFile );
        return FALSE;
    }
