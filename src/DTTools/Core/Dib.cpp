...
LPBYTE CDib::ReadBMPFile( LPCTSTR szFileName )
{
    ...
    {
        MessageBox( NULL, _T("打开位图文件出错!"), szFileName, MB_OK );
        return NULL;
    }
    // Read the header
    ...
    {
        CloseHandle( hFile );
        MessageBox( NULL, _T("读位图文件头出错!"), szFileName, MB_OK );
        return NULL;
    }
    ...
    {
        CloseHandle( hFile );
        MessageBox( NULL, _T("未知的位图格式文件!"), szFileName, MB_OK );
        return NULL;
    }
    ...
    {
        CloseHandle( hFile );
        MessageBox( NULL, _T("位图内存重新分配出错!"), szFileName, MB_OK );
        return NULL;
    }
    ...
    if( (!ReadFile( hFile, lpDIB, sizeof(BITMAPINFOHEADER), &dwBytes, NULL )) || (dwBytes!=sizeof(BITMAPINFOHEADER)) )
    {
        CloseHandle( hFile );
        free( lpDIB );
        MessageBox( NULL, _T("读位图文件信息头出错!"), szFileName, MB_OK );
        return NULL;
    }
    if( ((LPBITMAPINFOHEADER)lpDIB)->biSize != sizeof( BITMAPINFOHEADER ) )
    {
        CloseHandle( hFile );
        free( lpDIB );
        MessageBox( NULL, _T("OS/2风格的位图不支持!"), szFileName, MB_OK );
        return NULL;
    }
    ...
    {
        CloseHandle( hFile );
        MessageBox( NULL, _T("重新分配位图信息头所需内存失败!"), szFileName, MB_OK );
        free( lpDIB );
        return NULL;
    }
    lpDIB = lpTemp;
    ...
    if( wPaletteSize != 0 )
    {
        if( (!ReadFile( hFile, ((LPBITMAPINFO)lpDIB)->bmiColors, wPaletteSize, &dwBytes, NULL )) || (dwBytes!=wPaletteSize) )
        {
            CloseHandle( hFile );
            free( lpDIB );
            MessageBox( NULL, _T("读位图颜色表出错!"), szFileName, MB_OK );
            return NULL;
        }
    }
    // Seek to the bits
    ...
    if( bfh.bfOffBits != 0 )
    {
        if( SetFilePointer( hFile, bfh.bfOffBits, NULL, FILE_BEGIN ) == 0xffffffff )
        {
            CloseHandle( hFile );
            free( lpDIB );
            MessageBox( NULL, _T("位图文件大小定位出错!"), szFileName, MB_OK );
            return NULL;
        }
    }
    // Read the image bits
    if( (!ReadFile( hFile, FindDIBBits((LPSTR)lpDIB), dwBitsSize, &dwBytes, NULL )) || (dwBytes!=dwBitsSize) )
    {
        CloseHandle( hFile );
        free( lpDIB );
        MessageBox( NULL, _T("读位图文件出错!"), szFileName, MB_OK );
        return NULL;
    }
    // clean up
    CloseHandle( hFile );
...
BOOL CDib::WriteBMPFile( LPCTSTR szFileName, LPBYTE lpDIB )
{
    ...
    {
        MessageBox( NULL, _T("位图文件建立失败！"), szFileName, MB_OK );
        return FALSE;
    }
    bfh.bfType = 0x4d42;
    bfh.bfReserved1 = 0;
    ...
    {
        CloseHandle( hFile );
        MessageBox( NULL, _T("写位图文件头出错!"), szFileName, MB_OK );
        return FALSE;
    }
    ...
    {
        CloseHandle( hFile );
        MessageBox( NULL, _T("写位图文件出错!"), szFileName, MB_OK );
        return FALSE;
    }
    lpbmih->biHeight *= 2;
    CloseHandle( hFile );
